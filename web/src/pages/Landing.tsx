...
import { Search, ChevronRight, ChevronLeft, BarChart2, TrendingUp } from 'lucide-react'
...
function Landing() {
    ...
    useEffect(() => {
        supabase.from('games_2025_2026').select('*')
            .or('equipa_casa.ilike.%PORTO%,equipa_casa.ilike.%Benfica%,equipa_casa.ilike.%Sporting%,equipa_casa.ilike.%GAIA%,equipa_casa.ilike.%Ovarense%,equipa_casa.ilike.%Belenenses%,equipa_casa.ilike.%Academica%,equipa_casa.ilike.%Farense%')
            ...
            .then(({ data }) => {
                if (data && data.length > 0) {
                    const d = data as Match[]
                    setGames(d.filter(m => POPULAR_TEAMS.some(t => (m.equipa_casa + ' ' + m.equipa_fora).toUpperCase().includes(t.toUpperCase()))))
                }
    ...
    useEffect(() => {
        ...
        setResults(normalizedClubs.filter(c => c._n.includes(q)).slice(0, 20))
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        else if (e.key === 'Enter' && selectedIdx >= 0) { e.preventDefault(); selectClub(results[selectedIdx]) }
    ...
    const scrollCarousel = (dir: number) => {
        ...
        const scroll = carouselRef.current.scrollLeft + dir * 292
        carouselRef.current.scrollTo({ left: scroll, behavior: 'smooth' })
        setCarouselScroll(scroll)
    }

    const maxScroll = carouselRef.current ? carouselRef.current.scrollWidth - carouselRef.current.clientWidth : 0
    const canScrollRight = carouselScroll < maxScroll

    const clubLink = (slug: string) => '/clube/' + slug + '/home'
        ...
        <div className="pb-24">
                ...
                <div className="max-w-2xl mx-auto px-4 pt-16 md:pt-20 pb-10 text-center relative">
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mb-6 animate-slide-up">
                        {FEATURED_CLUBS.map(name => {
                            ...
                            if (!c) return null
                            return (<button key={c.slug} onClick={() => selectClub(c)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:border-dribly-purple/30 hover:text-dribly-purple hover:shadow-sm transition-all"><span className="w-5 h-5 rounded-full bg-dribly-purple/10 dark:bg-dribly-purple/20 flex items-center justify-center text-[9px] font-bold text-dribly-purple shrink-0">{name.charAt(0).toUpperCase()}</span>{name}</button>)
                        })}
                    </div>
                    <div className="max-w-lg mx-auto relative animate-slide-up" ref={dropdownRef}>
                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none"><Search size={20} className="text-zinc-400" /></div>
            ...
            <div className="bg-white dark:bg-zinc-950 border-b border-zinc-100 dark:border-white/5">
                <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-center gap-6 md:gap-16">
                    <div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">79</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Clubes</p></div>
                    <div className="w-px h-8 bg-zinc-200 dark:bg-white/10" />
                    <div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">411</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Competições</p></div>
                    <div className="w-px h-8 bg-zinc-200 dark:bg-white/10" />
                    <div className="text-center"><span className="text-lg font-black text-zinc-900 dark:text-white">24</span><p className="text-[10px] text-zinc-500 uppercase tracking-wide mt-0.5">Associações</p></div>
                </div>
            </div>
            <div className="py-8">
                <div className="max-w-2xl mx-auto px-4 mb-4"><div className="flex items-center justify-between"><h2 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-dribly-purple animate-pulse" />Jogos em Destaque</h2><div className="flex gap-1"><button onClick={() => scrollCarousel(-1)} disabled={carouselScroll === 0} className="p-1.5 rounded-full text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronLeft size={16} /></button><button onClick={() => scrollCarousel(1)} disabled={!canScrollRight} className="p-1.5 rounded-full text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-white/5 disabled:opacity-30 transition-colors"><ChevronRight size={16} /></button></div></div></div>
                <div className="max-w-2xl mx-auto px-4 relative">
                    {gamesLoading ? (<div className="flex gap-3 overflow-hidden">{[1,2,3].map(i => <div key={i} className="min-w-[280px] h-44 rounded-2xl bg-zinc-100 dark:bg-zinc-900 animate-pulse shrink-0" />)}</div>) : games.length === 0 ? (<p className="text-xs text-zinc-400 text-center py-8">Nenhum jogo em destaque de momento.</p>) : (<div className="relative"><div ref={carouselRef} onScroll={() => carouselRef.current && setCarouselScroll(carouselRef.current.scrollLeft)} className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">{games.map(match => (<div key={match.slug || match.id} className="min-w-[280px] shrink-0"><GameCard match={match} mode="agenda" /></div>))}</div><div className="absolute top-0 right-0 bottom-2 w-16 bg-gradient-to-l from-zinc-50 dark:from-zinc-950 to-transparent pointer-events-none" /></div>)}
                </div>
            </div>
            <div className="max-w-2xl mx-auto px-4 py-4">
                <div className="grid grid-cols-2 gap-3">
                    <Link to="/standings" className="glass-card p-4 flex items-center gap-3 hover:border-dribly-purple/20 group"><div className="w-10 h-10 rounded-xl bg-dribly-purple/10 dark:bg-dribly-purple/20 flex items-center justify-center shrink-0"><BarChart2 size={20} className="text-dribly-purple" /></div><div><h3 className="text-sm font-bold text-zinc-900 dark:text-white group-hover:text-dribly-purple transition-colors">Classificações</h3><p className="text-[10px] text-zinc-500">Consultar tabelas</p></div></Link>
                    <Link to="/about" className="glass-card p-4 flex items-center gap-3 hover:border-dribly-purple/20 group"><div className="w-10 h-10 rounded-xl bg-dribly-purple/10 dark:bg-dribly-purple/20 flex items-center justify-center shrink-0"><TrendingUp size={20} className="text-dribly-purple" /></div><div><h3 className="text-sm font-bold text-zinc-900 dark:text-white group-hover:text-dribly-purple transition-colors">Sobre</h3><p className="text-[10px] text-zinc-500">Como funciona</p></div></Link>
                </div>
            </div>
        </div>
    )
