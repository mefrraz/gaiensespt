...
function Landing() {
    ...
    useEffect(() => { const clubNames = ['FC Porto','SL Benfica','Sporting CP','UD Oliveirense']; const orClauses = clubNames.map(n => 'equipa_casa.ilike.%' + encodeURIComponent(n.replace(/ /g,'+')) + '%,equipa_fora.ilike.%' + encodeURIComponent(n.replace(/ /g,'+')) + '%').join(','); supabase.from('games_2025_2026').select('*').or(orClauses).neq('status','FINALIZADO').not('escalao','ilike','%Sub%').gte('data', new Date().toISOString().split('T')[0]).order('data',{ascending:true}).then(({data}:{data:any})=>{if(data){let arr=data as Match[];const counts:Record<string,number>={};arr=arr.filter(m=>{const match=clubNames.find(c=>(m.equipa_casa+' '+m.equipa_fora).toUpperCase().includes(c.toUpperCase()));if(!match)return false;counts[match]=(counts[match]||0)+1;return counts[match]<=2}).sort(()=>Math.random()-0.5).slice(0,8);setGames(arr)}setGamesLoading(false)}) },[])
