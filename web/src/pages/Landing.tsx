...
function Landing() {
    ...
    useEffect(() => { supabase.from('games_2025_2026').select('*').or('equipa_casa.ilike.%PORTO%,equipa_casa.ilike.%Benfica%,equipa_casa.ilike.%Sporting%,equipa_casa.ilike.%Oliveirense%').neq('status','FINALIZADO').not('escalao','ilike','%Sub%').gte('data', new Date().toISOString().split('T')[0]).order('data',{ascending:true}).then(({data}:{data:any})=>{if(data){let arr=data as Match[];const counts:Record<string,number>={};arr=arr.filter(m=>{const clubes=['PORTO','BENFICA','SPORTING','OLIVEIRENSE'];const match=clubes.find(c=>(m.equipa_casa+' '+m.equipa_fora).toUpperCase().includes(c));if(!match)return false;counts[match]=(counts[match]||0)+1;return counts[match]<=2}).sort(()=>Math.random()-0.5).slice(0,8);setGames(arr)}setGamesLoading(false)}) },[])
