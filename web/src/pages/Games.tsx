...
import { Calendar, Trophy, Filter, Loader2, MapPin, ChevronRight, Clock, RefreshCw, TrendingUp, TrendingDown, Minus } from 'lucide-react'
...
function Games() {
    ...
    useEffect(() => {
        const fetchMatches = async () => {
            ...
            if (view === 'agenda') {
                // Agenda: Upcoming games (not FINALIZADO) or just date sorting? 
                // Let's stick to status. Agenda = Not Finished. Results = Finished.
                // This keeps it clean.
    ...
    const formatTeamName = (name: string) => name.toUpperCase()
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-24">
            ...
            {/* Page Header is hidden in favor of Segment Controller acting as header */}

            {/* Segment Controller */}
            <div className="sticky top-20 z-40 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl p-1.5 rounded-2xl border border-zinc-200 dark:border-white/10 flex gap-1 shadow-xl mx-1 max-w-md mx-auto mt-4">
                <button
                ...
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all duration-300 ${view === 'results' ? 'bg-zinc-100 dark:bg-zinc-800 text-black dark:text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-300'}`}
                >
                    ...
                    {sortedDates.length === 0 ? (
                        ...
                        sortedDates.map(date => (
                            <div key={date} className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                                <h3 className="text-xs font-bold text-zinc-500 mb-3 uppercase tracking-widest pl-2">
                                    {formatDate(date)}
                                </h3>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {view === 'agenda' ? (
                                                            <>
                                                                ...
                                                                <span className="text-xs font-mono font-bold tracking-wider text-gaia-yellow">
