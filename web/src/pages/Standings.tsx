...
const ASSOCIATION_LOGOS: Record<number, string> = {
    50: 'fpb.jpg',      1: 'ablisboa.jpg',     2: 'absetubal.jpg',
    3: 'abaveiro.jpg',  4: 'abporto.jpg',      5: 'abbraga.jpg',
    6: 'abmadeira.jpg', 7: 'absantarem_novo.jpg', 8: 'abcoimbra.jpg',
    9: 'abalgarve.jpg', 10: 'abviseu.jpg',     11: 'ableiria.jpg',
    12: 'abalentejo.jpg', 13: 'abit.jpg',      14: 'abcastelobranco.jpg',
    15: 'abbraganca.jpg', 16: 'absaomiguel.jpg', 17: 'abviana.jpg',
    18: 'abvilareal.jpg', 19: 'abifp.jpg',     20: 'abguarda.jpg',
    22: 'absantamaria.jpg', 24: 'abacores.jpg',
}

function logoUrl(id: number): string {
    const f = ASSOCIATION_LOGOS[id]
    return f ? `${TUGABASKET_ASSETS}/${f}` : ''
}
...
export default function Standings() {
    ...
    async function loadAssociations(s: string) {
        ...
        const { data } = await supabase.from('competitions')
            .select('association_id, association_name').eq('season', s).order('association_name')
        if (data) {
            const seen = new Set<number>()
            const u: AssociationMeta[] = []
            for (const r of data) { const id = r.association_id as number; if (!seen.has(id)) { seen.add(id); u.push({ association_id: id, association_name: r.association_name as string }) } }
            setAssociations(u)
        }
    ...
    async function loadCompetitions(assocId: number) {
        ...
        const { data } = await supabase.from('competitions')
            ...
            .eq('season', season).eq('association_id', assocId).order('competition_name')
        if (data) {
            const comps = data.map(r => ({
                competition_id: r.competition_id as number,
                competition_name: r.competition_name as string,
                association_name: r.association_name as string,
                club_count: Array.isArray(r.club_names) ? (r.club_names as string[]).length : 0,
                gender: detectGender(r.competition_name as string),
            }))
            setCompetitions(comps)
            if (comps.some(c => c.gender === 'M')) setActiveTab('M')
            else if (comps.some(c => c.gender === 'F')) setActiveTab('F')
            else setActiveTab('O')
        }
        setCompsLoading(false)
    }

    function navigate(to: View, fn?: () => void) {
        setAnimating(true)
        setTimeout(() => { fn?.(); setView(to) }, 120)
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 dark:from-[#09090b] dark:via-zinc-950 dark:to-[#09090b] pb-16">
            <div className="max-w-6xl mx-auto px-3 sm:px-5 md:px-8 pt-6 sm:pt-8">
                {/* ── HEADER ── */}
                <div className={`mb-6 sm:mb-8 transition-all duration-300 ${animating ? 'opacity-40 scale-[0.995]' : 'opacity-100 scale-100'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
                        ...
                        <div className="relative shrink-0 self-start sm:self-auto">
                            <select value={season} onChange={e => { setSeason(e.target.value); setView('associations') }}
                ...
                {view === 'associations' && (
                    <div className={`transition-all duration-300 ${animating ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
                        {assocsLoading ? (
                            ...
                            <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2.5 sm:gap-3.5">
                                {associations.map(a => (
                                    <button key={a.association_id}
                                        onClick={() => navigate('competitions', () => { setSelectedAssoc(a); loadCompetitions(a.association_id) })}
                                        className="group bg-white dark:bg-zinc-900/90 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 p-3.5 sm:p-4 shadow-sm hover:shadow-xl hover:border-zinc-300 dark:hover:border-zinc-600 hover:-translate-y-0.5 transition-all duration-200 flex flex-col items-center gap-2.5 sm:gap-3">
                                        <div className="w-16 h-16 sm:w-[72px] sm:h-[72px] rounded-2xl bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-800 dark:to-zinc-800/50 flex items-center justify-center p-2.5 sm:p-3 border border-zinc-100 dark:border-zinc-700/50 shadow-inner group-hover:shadow-md transition-all duration-200">
                                        ...
                                        {[{ label: 'Masculinas', key: 'M' as const, data: masculine },
                                          ...
                                          .map(t => (
                                            <button key={t.key} onClick={() => setActiveTab(t.key)}
                                                ...
                                                {t.label}
                                                <span className="ml-1.5 opacity-50 text-[10px]">{t.data.length}</span>
                                            </button>
                                        ))}
                                    </div>
                                )}
                                ...
                                {(() => {
                                    ...
                                    if (!list.length) return (
                                        <div className="text-center py-20 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60">
                                            <p className="text-zinc-400 font-medium">Nenhuma competição nesta categoria.</p>
                                        </div>
                                    )
                                    return (
                                        <div className="space-y-2">
                                            {list.map(c => (
                                                <button key={c.competition_id}
                                                    ...
                                                    <div className="min-w-0">
                                                        <h4 className="text-sm sm:text-base font-bold text-zinc-800 dark:text-zinc-200 group-hover:text-amber-600 dark:group-hover:text-amber-500 transition-colors leading-snug">
                                                            {c.competition_name}
                                                        </h4>
                                                        <p className="text-[11px] sm:text-xs text-zinc-400 mt-1">{c.association_name}</p>
                                                    </div>
                                                    <div className="shrink-0 flex flex-col items-end gap-1">
                                                        <span className="text-xs font-bold text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-2.5 py-1 rounded-full">
                                                            {c.club_count} equipas
                                                        </span>
                                                        <svg className="w-4 h-4 text-zinc-300 dark:text-zinc-600 group-hover:text-amber-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
                                                    </div>
                                                </button>
                                            ))}
                                        </div>
                        ...
                        {stLoading ? (
                            ...
                            <div className="space-y-3">
                                {groups.map(g => {
                                    ...
                                    return (
                                        <StandingsTable key={g} grupo={g} teams={t}
                                            isOpen={isOpen(g)} onToggle={() => toggleGroup(g)}
                                            status={getStatus(t)} />
                                    )
                                })}
                            </div>
                ...
                <div className="mt-12 flex flex-wrap gap-4 text-[10px] font-medium text-zinc-400 justify-center">
